...
import { supabase, supabaseOperation, subscribeToProducts, subscribeToCategories, subscribeToBookings, subscribeToCustomerCodes, subscribeToMessages, uploadToSupabase } from './lib/supabase';
...
function UserSettingsView({ user, isDarkMode, onLogout }: any) {
  const [language, setLanguage] = useState('ar');
  return (
    <div className={isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'} style={{padding: 32, borderRadius: 16, margin: 32}}>
      <h2 className="text-2xl font-bold mb-4">إعدادات المستخدم</h2>
      <p>مرحباً {user?.customer_name || 'مستخدم'}!</p>
      <button onClick={onLogout} className="mt-4 px-4 py-2 rounded bg-pink-600 text-white">تسجيل الخروج</button>
    </div>
  );
}
  ...
  useEffect(() => {
    const setupSync = async () => {
      try {
        ...
        if ((codesRes as any).success) {
          setCustomerCodes((codesRes as any).data || []);
        } else {
          setCustomerCodes([]);
          if ((codesRes as any).error && ((codesRes as any).error.includes('column') || (codesRes as any).error.includes('field'))) {
            showToast('⚠️ هناك حقل ناقص في قاعدة البيانات. الرجاء مراجعة لوحة Supabase وإضافة الحقول المطلوبة في جدول customer_codes.', 'error');
          }
        }
        if ((bksRes as any).success) setBookings((bksRes as any).data || []);
        else setBookings([]);
    ...
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-gray-950 text-white' : 'bg-white text-gray-900'} transition-all duration-300`}>
      ...
      <main className="flex-1 overflow-x-hidden">
        <Routes>
          ...
          <Route path="/admin/*" element={<ProtectedRoute role={role} adminOnly><AdminRoutes products={products} categories={categories} bookings={bookings} codes={customerCodes} content={content} onRefresh={refreshData} isDarkMode={isDarkMode} /></ProtectedRoute>} />
          ...
          <Route path="/settings" element={<ProtectedRoute role={role}><UserSettingsView user={userSession} isDarkMode={isDarkMode} onLogout={handleLogout} /></ProtectedRoute>} />
...
function AdminCategoryManager({ categories, onRefresh, isDarkMode }: any) {
  ...
  const handleSaveEdit = async () => {
    ...
    await supabaseOperation('update', 'categories', { name: editedName }, editingId);
    setEditingId(null);
    setEditedName('');
    onRefresh();
  };
  ...
  const updateStatus = async (id: string, status: string, code: string, isLocal?: boolean, customerName?: string, userPhone?: string) => {
    try {
      if (isLocal) {
        ...
        const result = await supabaseOperation('update', 'customer_codes', { status, is_active: isActive, code: isActive ? code : 'PENDING' }, id);
        if (status === 'approved') {
...
function AccessGateView({ onVerify, onScan, onRequest, isDarkMode }: any) {
  ...
  const validatePhoneNumber = (phone: string, country: string) => {
    ...
    switch (country) {
      case 'YE': // Yemen
        return cleanPhone.length === 9 && ['77', '78', '73', '71', '70'].some(prefix => cleanPhone.startsWith(prefix));
  ...
  const handleRequest = () => {
    ...
    if (!validatePhoneNumber(phone, selectedCountry)) {
      const messages = {
        YE: 'رقم الهاتف اليمني يجب أن يكون 9 أرقام ويبدأ بـ 77 أو 78 أو 777 أو 73 أو 71 أو 70',
        SA: 'رقم الهاتف السعودي يجب أن يكون 9 أرقام ويبدأ بـ 5',
        US: 'رقم الهاتف الأمريكي يجب أن يكون 10 أرقام'
      };
      alert(messages[selectedCountry as keyof typeof messages]);
      return;
    }

    const fullPhone = selectedCountry === 'YE' ? `+967${phone}` :
                     selectedCountry === 'SA' ? `+966${phone}` :
                     `+1${phone}`;
...
function NotificationsView({ userId, isDarkMode }: any) {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      try {
        ...
        const res = await supabaseOperation('get', 'notifications', { user_id: userId });
        if ((res as any).success) {
  ...
  const markAsRead = async (id: string) => {
    try {
      await supabaseOperation('update', 'notifications', { is_read: true }, id);
